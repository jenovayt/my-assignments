...
function logNames(INVENTORY) {
  ...
  function getUppercaseNames(items) { 
    const uppercaseLetters = items.map((item) => {
  ...
  function getItemById(items, id) {
    const groceryId = items.find ((item) => {
      return item.id;
    })
    return groceryId
  }
  ...
  function getItemsByCategory(items, category) {
    const groceryCategory = items.filter(item => item.category === category)
  ...
  function calculateTotalPrice(items) {
    return items.reduce(
      (totalPrice, currentItem) => {
        const totalItemPrice = currentItem.price * currentItem.quantity
        return totalItemPrice + totalPrice
      }, 0
    )
  }
